export default async function handler(req, res) {
  ...
  try {
    ...
    // --- Date Range Setup for the last 30 days ---
    ...
    if (!totalInsightsResponse.ok) throw new Error(`Failed to fetch account insights: ${totalInsightsResponse.statusText}`);
    ...
    if (!dailyInsightsResponse.ok) throw new Error(`Failed to fetch daily insights: ${dailyInsightsResponse.statusText}`);
    ...
    const campaignStatuses = ['ACTIVE', 'PAUSED', 'ARCHIVED'];
